/**
 * Clipboard Proxy
 * 剪贴板代理 - 在 Android 后台时通过 IME / Shizuku / 悬浮窗获取剪贴板，其他情况直接调用 expo-clipboard
 *
 * 优先级：IME > Shizuku > 悬浮窗 > 直接调用
 *
 * IME 模式利用 Android 输入法特权实现无感后台剪贴板监听，无需额外权限。
 * 悬浮窗按需显示（不可见的 1px 窗口），
 * 若持续 10 秒无后台剪贴板调用，自动关闭悬浮窗以节省资源。
 */
...
let imeModule: typeof import('clipboard-ime') | null = null;
...
if (Platform.OS === 'android') {
  ...
  imeModule = require('clipboard-ime');
...
/**
 * 判断是否应该使用 IME 获取剪贴板（最高优先级）
 * 条件：Android + 后台 + 设置启用 + IME 已启用
 */
async function shouldUseIme(): Promise<boolean> {
  if (Platform.OS !== 'android' || !imeModule) return false;
  if (AppState.currentState !== 'background') return false;
  const config = await configService.getConfig();
  if (!(config?.enableImeClipboard ?? false)) return false;
  if (!imeModule.isImeEnabled()) return false;
  return true;
}
...
export async function getStringAsync(options?: Clipboard.GetStringOptions): Promise<string> {
  if (await shouldUseIme()) {
    try {
      const result = await imeModule!.getImeClipboardText();
      return result;
    } catch (e) {
      console.warn('[ClipboardProxy] IME getStringAsync failed, falling back:', e);
    }
  }
...
export async function hasStringAsync(): Promise<boolean> {
  if (await shouldUseIme()) {
    try {
      return await imeModule!.hasImeClipboardText();
    } catch (e) {
      console.warn('[ClipboardProxy] IME hasStringAsync failed, falling back:', e);
    }
  }
  if (await shouldUseShizuku()) {
    try {
...
export async function hasImageAsync(): Promise<boolean> {
  if (await shouldUseIme()) {
    try {
      return await imeModule!.hasImeClipboardImage();
    } catch (e) {
      console.warn('[ClipboardProxy] IME hasImageAsync failed, falling back:', e);
    }
  }
  if (await shouldUseShizuku()) {
    try {
...
export async function getImageAsync(
  ...
  if (await shouldUseIme()) {
    try {
      const uri = await imeModule!.getImeClipboardImageUri();
      if (uri) {
        return null;
      }
    } catch (e) {
      console.warn('[ClipboardProxy] IME getImageAsync failed, falling back:', e);
    }
  }
  if (await shouldUseOverlay()) {
    try {
...
export async function saveImageToFileAsync(
  ...
  if (await shouldUseIme()) {
    try {
      const result = await imeModule!.saveImeClipboardImageToFile(destDirPath);
      if (result) return { filePath: result.filePath, mimeType: result.mimeType };
    } catch (e) {
      console.warn('[ClipboardProxy] IME saveImageToFileAsync failed, falling back:', e);
    }
  }
  if (await shouldUseOverlay()) {
    try {
