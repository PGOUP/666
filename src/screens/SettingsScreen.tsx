...
export const SettingsScreen = () => {
  ...
  const [localImeClipboardEnabled, setLocalImeClipboardEnabled] = useState(
    config?.enableImeClipboard ?? false
  );
  ...
  // 处理切换 IME 输入法获取剪贴板
  const handleToggleImeClipboard = async (enabled: boolean) => {
    if (enabled && Platform.OS === 'android') {
      const { isImeEnabled, openImeSettings } = await import('clipboard-ime');

      if (!isImeEnabled()) {
        Alert.alert(
          '需要启用 SyncClipboard IME',
          '要使用输入法方式获取剪贴板，需要先在系统设置中启用 SyncClipboard IME 输入法。\n\n启用后无需将其设为默认输入法，只需在输入法列表中开启即可。\n\n点击"前往设置"后，在"屏幕键盘"或"虚拟键盘"中找到并启用 SyncClipboard IME。',
          [
            { text: '取消', style: 'cancel' },
            {
              text: '前往设置',
              onPress: () => {
                ...
                Alert.alert(
                  '操作说明',
                  '在输入法设置页面中：\n\n1. 点击"屏幕键盘"或"虚拟键盘"\n2. 找到"SyncClipboard IME"并开启\n3. 返回后再次打开此开关即可\n\n提示：无需将 SyncClipboard IME 设为默认输入法',
                  [{ text: '知道了' }]
                );
              },
            },
      ...
      setLocalImeClipboardEnabled(true);
      try {
        if (localShizukuClipboardEnabled) {
          setLocalShizukuClipboardEnabled(false);
          await setEnableShizukuClipboard(false);
        }
        if (localClipboardOverlayEnabled) {
          setLocalClipboardOverlayEnabled(false);
          await setEnableClipboardOverlay(false);
        }
        await setEnableImeClipboard(true);
        showMessage('已启用 IME 输入法获取剪贴板', 'success');
      } catch (error: unknown) {
        setLocalImeClipboardEnabled(false);
        showMessage(error instanceof Error ? error.message : '设置失败', 'error');
      }
      return;
    }
          ...
          <View style={styles.section}>
            ...
            >
              ...
              <View style={[styles.settingRow, { borderBottomColor: theme.colors.divider }]}>
                ...
                <Switch
                  value={localBackgroundTasksEnabled && localImeClipboardEnabled}
                  onValueChange={handleToggleImeClipboard}
                  trackColor={{ false: theme.colors.divider, true: theme.colors.success }}
                  thumbColor={
                    localBackgroundTasksEnabled && localImeClipboardEnabled
                      ? theme.colors.surface
                      : theme.colors.textTertiary
                  }
                  disabled={!localBackgroundTasksEnabled}
